...
* **IP-Based Blocking & Device Protection** — A dedicated security filter monitors authentication attempts and applies a dynamic, temporary block based on the user's IP address when repeated failed logins are detected, preventing malicious disruptions for legitimate users.
...
* **Retrieval Pipeline**:
  * Receive the pharmacist's question.
  * Generate a semantic embedding for the query.
  * Search Qdrant for the most relevant FDA passages.
  * Rerank candidates using FlashRank to improve retrieval precision.
  * Build an augmented prompt from the reranked context.
  * Generate a context-grounded answer using Llama 3.3 via the Groq API.
  * Return the answer together with its supporting source evidence.
...
The backend is an enterprise-grade ASP.NET Core MVC application, ensuring maintainability, separation of concerns, and testability.
...
* **External Service Outage Handling**: Designed to maintain a smooth user experience; if any external dependent service becomes unavailable, the system gracefully handles the failure by displaying a clear, user-friendly error message to the user rather than failing silently.
...
* **Dual-Strategy Concurrency Control Layer**: To maximize system throughput while maintaining absolute data integrity, the system splits its concurrency management into two distinct strategies based on contention probability:
  * **Pessimistic Locking (High-Contention / General Dispensing)**: Engineered for scenarios with a high probability of race conditions—such as multiple pharmacists across different branches simultaneously dispensing the same high-demand drug from the same inventory batch. During checkout, the infrastructure layer utilizes EF Core to execute explicit raw SQL row-level locking hints on SQL Server (`SELECT ... WITH (UPDLOCK, ROWLOCK)`). This acquires an update lock on the targeted inventory row, causing concurrent transactions attempting to modify the same record to wait until the active transaction completes, thereby preventing concurrent stock modification conflicts and eliminating double-selling or negative-stock anomalies.
...
* **ASP.NET Core Identity** — Manages authentication, password hashing, cookies, and role-based access control, with dedicated Administrator and Pharmacist roles to enforce secure, permission-based access across the system.
...
* **Brevo API** — SMTP provider for token delivery and automated reporting.
...
![C4 Architecture Diagram](https://Diagram-C4.png)

---

## 📺 Video Demo & System Walkthrough

[![Watch Demo Video](https://img.shields.io/badge/Watch_Demo_Video-Google_Drive-blue?style=for-the-badge&logo=googledrive&logoColor=white)](https://img.shields.io/badge/Watch_Demo_Video-Google_Drive-blue?style=for-the-badge&logo=googledrive&logoColor=white)

Click the badge above or the direct link in the repository header to watch a video demonstration covering the system's core capabilities and high-performance engineering:
